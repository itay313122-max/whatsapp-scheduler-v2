function App() {
  ...
  return (
    <div className="app-shell">
      <style>{`:root{--c-from:#2563EB;--c-to:#1E3A8A;--c-primary:#2563EB;--c-primary-light:rgba(37,99,235,0.1);--c-bg:#f4f7ff;--c-font:'Inter',system-ui,sans-serif;}`}</style>
      ...
      <div className="app-content">
        ...
        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <svg width="128" height="128" viewBox="0 0 128 128" style={{ flexShrink: 0 }}>
            ...
            <text x="64" y="60" textAnchor="middle" fontSize="22" fontWeight="800" fill="#1e293b">{mm}:{ss}</text>
            <text x="64" y="80" textAnchor="middle" fontSize="10" fill="#64748b">{Math.round(pct * 100)}% of goal</text>
          </svg>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 2 }}>Daily Goal</div>
            <div style={{ color: '#64748b', fontSize: 12, marginBottom: 10 }}>45 min of active training</div>
            <button className="btn-primary" style={{ width: '100%' }} onClick={() => setRunning(r => !r)}>
              {running ? '⏸ Pause' : sec ? '▶ Resume' : '▶ Start workout'}
            </button>
          </div>
        </div>
        <div style={{ fontWeight: 700, fontSize: 13, color: '#64748b', margin: '4px 2px' }}>Recommended for you</div>
        {WORKOUTS.map(w => (
