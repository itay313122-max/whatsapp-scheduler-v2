  ...
  async function handleSocialAuth(provider: 'google' | 'github') {
    ...
    try {
      ...
      if (msg.includes('not configured')) {
        ...
        setError(`${provider === 'google' ? 'Google' : 'GitHub'} sign-in failed — try "Continue as guest"`);
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="glass-card rounded-3xl p-8">
        ...
        <div className="space-y-3 mb-6">
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 py-3.5 rounded-2xl bg-[#24292f] border border-[#24292f] hover:bg-[#2f363d] text-white text-sm font-semibold transition-all disabled:opacity-50"
          >
