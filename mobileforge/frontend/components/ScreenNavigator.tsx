  ...
  return (
    <div className="flex flex-col h-full" dir="ltr">
      <div className="px-3 py-2 border-b border-border flex items-center gap-2">
        ...
        <span className="text-xs font-semibold text-text-secondary uppercase tracking-wide">Screens</span>
      </div>
      ...
      <div className="flex-1 overflow-auto py-1">
        {screens.length > 0 ? (
          ...
          <div className="px-3 py-4 text-xs text-text-soft text-center">
            Build an app to see its screens
          </div>
        )}
      </div>
      ...
      <div className="border-t border-border p-2">
        <p className="text-[10px] text-text-soft uppercase tracking-wide mb-1.5 px-1">Add screen</p>
        <div className="grid grid-cols-2 gap-1">
          {PRESET_SCREENS.map((s) => (
            <button
              key={s.id}
              onClick={() => onAddScreen(`Add a ${s.label} screen to the app`)}
